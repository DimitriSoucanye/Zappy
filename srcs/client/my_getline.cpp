...
** Last update Mon Jun 19 00:54:12 2017 Antoine Galpin
*/
...
static char *duplol(const char *src) {
  ...
  while (src != NULL && src[xd])
    xd += 1;
  if ((new_str = (char *)malloc(sizeof(char) * xd + 1)) == NULL)
    return (NULL);
  xd = 0;
  while (src[xd] != '\0') {
    new_str[xd] = src[xd];
...
static char *copy(char *dest, const char *src, unsigned int nbr, int conc) {
  unsigned int xd;
  unsigned int xd_src;
...
static char *_auto_alloc(char *ptr, size_t mem) {
  ...
  while (ptr != NULL && ptr[ptr_len])
    ptr_len += 1;
  if (ptr == NULL) {
    if ((new_ptr = (char *)malloc(sizeof(char) * mem)) == NULL)
      return (NULL);
    if (mem > 0)
      new_ptr[0] = '\0';
    return (new_ptr);
  }
  if ((new_ptr = (char *)malloc(sizeof(char) * (ptr_len + 1 + mem))) == NULL)
    return (NULL);
...
static char *_get_new_buffer(char *buf, char *buf_full, const int fd,
                             int eols) {
  int tmp;
...
char *my_getline(const int fd) {
  ...
  xd_buf = 0;
  if (READ_SIZE <= 0 ||
      (buf = (char *)malloc(sizeof(char) * (READ_SIZE + 1))) == NULL ||
